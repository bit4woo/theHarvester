...
def usage():
    ...
    print """       -b: data source: google, googleCSE, bing, bingapi, pgp, linkedin,
                        google-profiles, jigsaw, twitter, googleplus, default(google,baidu,bing,yahoo)\n"""
    ...
    print "       -k: The SHODAN API key"
    print "       -p: The http proxy for google,linkedin,twitter. eg. -p http://127.0.0.1:8080"
    print "\nExamples:"
    print "        " + comm + " -d jd.com -b default -h"
    print "        " + comm + " -d jd.com -b default -h -nct"
...
def start(argv):
    if len(sys.argv) < 4:
        ...
        sys.exit()
    try:
        opts, args = getopt.getopt(argv, "l:d:b:s:vf:nhcte:k:p:")
    ...
    for opt, arg in opts:
        if opt == '-l':
            ...
            proxy = {arg.split(":")[0]:arg}
        elif opt == '-k':
            shodankey = arg
        elif opt == '-b':
            engine = arg
            if engine not in ("google","googleCSE" , "linkedin", "pgp", "google-profiles", "bing", "bingapi",
                              "yandex", "jigsaw", "dogpilesearch", "twitter", "googleplus", "yahoo", "baidu", "default"):
                usage()
    ...
    if engine == "google":
        ...
        search = googlesearch.search_google(word, limit, start, proxy)
        search.process()
    ...
    if engine == "googleCSE":
        ...
        search = googleCSE.search_googleCSE(word, limit, start, proxy)
        search.process()
    ...
    if engine == "exalead":
        ...
        search = twittersearch.search_twitter(word, limit, proxy)
        search.process()
        people = search.get_people()
        ...
        search = linkedinsearch.search_linkedin(word, limit, proxy)
        search.process()
        people = search.get_people()
        ...
        for x in full:
            ...
            try:
                ...
                if not shodanvisited.count(ip):
                    ...
                    a = shodansearch.search_shodan(ip,shodankey)
